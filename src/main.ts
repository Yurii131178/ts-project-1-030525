...
user2.name = "Poly"; // ✅ це поле можна змінювати
...
console.log(names3[0].length);
console.log(names3[1].toUpperCase());
...
interface User4 {
  // або альтернативний Array<User4>
  name4: string;
  age5: number;
}
...
// 📦 Приклад з типами // літеральні типи
...
//.................................................//
console.log("****Задача: Обробка типу доставки*****");
...
function getDeliveryMessage(delivery: DeliveryType) {
  ...
  } else return "Замовлення буде доставлено у поштомат";
}
...
console.log(getDeliveryMessage("locker")); // => Замовлення буде доставлено у поштомат
...
const electronicCategory = categories.data.find(
...
const emails = getUserEmails(userList);
console.log(names); // ['Alice', 'Bob', 'Charlie', 'Poly']
console.log(emails); // ['alib@com', 'bob@com', 'cha@com', 'poly@com']

//У цьому прикладі функція getUserNames/getUserEmails приймає масив об'єктів типу User та повертає масив рядків.
...
function giveAKick(name: string, isPresent?: boolean) {
  if (isPresent) {
    console.log(`You can kick ${name} `);
  } else {
    console.log(
      `${name} is not in the office today, so make a note to kick him when he's back`
    );
  }
}

giveAKick("Andriy", true);
giveAKick("Andriy");

//*****************************************//
console.log("===Function Type (Тип функції)===GENERICS У ФУНКЦІЯХ");
...
const muliply: AddFunction = (c, d) => c * d;
console.log(muliply(30, 5));
...
/**==========❌Типізація промісів❌==================== */
/**Типізація промісів
При роботі з асинхронними операціями, наприклад такими як HTTP-запити, отримаємо проміси, які також потрібно типізувати як Promise<T>, щоб вказати, який тип даних повертається після виконання проміса.
...
Тип Promise<T> вказує, що функція поверне результат типу T після завершення асинхронної операції. У цьому прикладі Promise<string> означає, що Promise виконається з даними тип яких буде рядок (string).
...
const getUser = (): Promise<User9> => {
  return new Promise((resolve) => {
    setTimeout(() => resolve({ id: 1, name: "Jojoba" }), 1000);
  });
};
...
getUsers().then((users) => console.log(users[3].name));
//...............................................................................//
...
const getProducts = (): Promise<Product1[]> => {
  return new Promise((resolve) => {
    resolve([
      { id: 1, title: "Phone", price: "200 UAH " },
      { id: 2, title: "Tablet", price: "300 UAH " },
      { id: 3, title: "Earpods", price: "400 UAH " },
      { id: 4, title: "Wecamera", price: "500 UAH " },
    ]);
  });
};

getProducts().then((products) =>
  console.log(products[3].title, `, Price:`, products[3].price)
);
//.............................................................................
// **===============❌Типізація HTTP-запитів❌===================================//

// У TypeScript при роботі з fetch або axios важливо правильно типізувати отримані дані, тобто проміс і його значення.
...
// Оператор as Тип означає, що ми очікуємо відповідь саме цієї структури. TypeScript довіряє, але не перевіряє вміст автоматично.

// Коли викликаємо await response.json(), то отримуємо data типу any(будь-який тип). TypeScript не знає, яку саме структуру повертає сервер – лише те, що це "щось з JSON". Це вимикає перевірку типів та призведе до помилок, які не видно під час розробки. Тому ми вказуємо TypeScript as User, що ми очікуємо об'єкт типу User.
...
// axios.get<User>(...) означає, що відповідь очікується у форматі User і це буде тип для значення властивості response.data.

// Якщо запит повертає масив:

const fetchUsers3 = async (): Promise<User[]> => {
